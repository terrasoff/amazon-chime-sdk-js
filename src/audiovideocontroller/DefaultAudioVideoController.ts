...
export default class DefaultAudioVideoController
  ...
  private actionFinishUpdating(): void {
    ...
    if (!this.meetingSessionContext.enableSimulcast) {
      ...
      const scaleResolutionDownBy = this.meetingSessionContext.videoUplinkBandwidthPolicy
  ...
  async handleHasBandwidthPriority(hasBandwidthPriority: boolean): Promise<void> {
    if (
      ...
      const oldScaleFactor = this.meetingSessionContext.videoUplinkBandwidthPolicy.scaleResolutionDownBy();
      ...
      const newScaleFactor = this.meetingSessionContext.videoUplinkBandwidthPolicy.scaleResolutionDownBy();

      if (oldMaxBandwidth !== newMaxBandwidth || oldScaleFactor !== newScaleFactor) {
        this.logger.info(
          `video send bandwidth priority ${hasBandwidthPriority} max has changed from ${oldMaxBandwidth} kbps and scale down resolution factor ${oldScaleFactor} to ${newMaxBandwidth} kbps and scale down resolution factor ${newScaleFactor}`
        );
